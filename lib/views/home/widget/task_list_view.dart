...
class TaskListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final taskFirestoreService = Provider.of<TaskFirestoreService>(
        context); // Access TaskFirestoreService

    return StreamBuilder<List<Task>>(
      stream: taskFirestoreService
          .getTasksForUser(), // Get tasks from Firestore as a stream
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return const Center(
              child: Text(
                  "No tasks available.")); // Show message if no tasks are found
        } else {
